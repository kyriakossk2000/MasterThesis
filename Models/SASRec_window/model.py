...
# for time2vec --> https://github.com/ojus1/Time2Vec-PyTorch
...
class SineActivation(torch.nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.w0 = torch.nn.parameter.Parameter(torch.randn(in_features, 1))
        self.b0 = torch.nn.parameter.Parameter(torch.randn(1))
        self.w = torch.nn.parameter.Parameter(torch.randn(in_features, out_features-1))
        self.b = torch.nn.parameter.Parameter(torch.randn(out_features-1))
        self.f = torch.sin
...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        self.uniform_ss = args.uniform_ss
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs): # for training        
        ...
        if not self.masking and self.model_training == 'all_action':
        ...
        elif self.model_training == 'all_action':
...
class SASRecT2V(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        super(SASRecT2V, self).__init__()
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs, time_seqs, pos_time): # for training        
        ...
        if self.model_training == 'combined':
            if pos_seqs.ndim == 3:
                ...
                if self.loss_type == 'ce_over':
                    ...
                    if self.strategy in ['autoregressive', 'teacher_forcing']:
                        seqs = torch.LongTensor(log_seqs).to(self.dev)  # to a PyTorch tensor
                        time_seqs = torch.FloatTensor(time_seqs).unsqueeze(-1).to(self.dev)


                        for i in range(max_window_size):

                            log_feats = self.log2feats(seqs, time_seqs)
                            ...
                            if i < pos_size:
                                ...
                                if self.strategy == 'autoregressive':
                                    predicted_action = pos_logits.argmax(dim=-1).unsqueeze(1)
                                    future_time = torch.zeros_like(predicted_action).unsqueeze(-1).to(self.dev)  # placeholder time of 0s since cannot predict time 

                                elif self.strategy == 'teacher_forcing':
                                    predicted_action = torch.LongTensor(pos_seqs[:, :, i]).to(self.dev)[:,-1].unsqueeze(1)
                                    future_time = torch.FloatTensor(pos_time[:, :, i]).to(self.dev)[:,-1].unsqueeze(-1).unsqueeze(1) # true time sequence appended 


                                seqs = seqs[:, 1:]  # remove the first element to maintain the embedding size
                                seqs = torch.cat([seqs, predicted_action], dim=1)
                                time_seqs = time_seqs[:, 1:]  # update the time_seqs tensor in a similar way
                                time_seqs = torch.cat([time_seqs, future_time], dim=1)
            ...
            if self.loss_type == 'ce_over':
                ...
                if self.strategy in ['autoregressive', 'teacher_forcing']:
                    seqs = torch.LongTensor(log_seqs).to(self.dev)  # to a PyTorch tensor
                    time_seqs = torch.FloatTensor(time_seqs).unsqueeze(-1).to(self.dev) # convert time_seqs to a PyTorch tensor
    ...
    def predict(self, user_ids, log_seqs, item_indices, time_seqs): # for inference
        log_feats = self.log2feats(log_seqs, time_seqs) # user_ids hasn't been used yet
